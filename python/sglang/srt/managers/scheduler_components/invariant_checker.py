...
class SchedulerInvariantChecker:
    ...
    # PD decode: requests parked in the prealloc/transfer/waiting queues hold
    # KV that is in no batch; count their uncached tokens too.
    get_extra_reqs: Callable = field(default=lambda: [])
    ...
    def _get_total_uncached_sizes(
        ...
        reqs.extend(self.get_extra_reqs())
        for req in reqs:
