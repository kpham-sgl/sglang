...
class TestDecodeLockRefScenarios(CustomTestCase):
    ...
    def test_swa_tail_len_keeps_window_below_bigram_insert_boundary(self):
        """An EAGLE bigram key over n tokens has n - 1 entries, so a page-multiple
        n inserts one page less than the unigram key; the SWA tail has to keep a
        full window below that lower boundary or the arrival insert is not
        matchable and its nodes are left unlocked."""
        enter_override(
            self,
            get_context().override_server_args(
                disaggregation_decode_enable_radix_cache=True
