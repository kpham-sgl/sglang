...
source "$D/env.sh"
alive() { pgrep -f "launch.sh decode" >/dev/null && curl -sf http://127.0.0.1:$DECODE_PORT/health >/dev/null; }
...
PY
    ...
    3) echo "[$(date -u +%T)] step3 hit-vs-cold"
       python3 "$D/hit_vs_cold.py" 12 /scratch/repro-out/$TAG.hit-vs-cold.json 2>&1 | tail -4
