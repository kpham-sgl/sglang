...
COMMON=(--model-path "$MODEL" --trust-remote-code --dtype bfloat16 --kv-cache-dtype bfloat16
  --tp-size "$TP" --page-size "${PAGE_SIZE:-64}" --mem-fraction-static "${MEM_FRAC:-0.6}"
...
case $ROLE in
  prefill)
    CUDA_VISIBLE_DEVICES=$PREFILL_GPUS exec sglang serve "${COMMON[@]}" --disaggregation-mode prefill \
      --disaggregation-bootstrap-port $BOOTSTRAP_PORT --port $PREFILL_PORT --nccl-port 30300 ;;
  decode)
    ...
    CUDA_VISIBLE_DEVICES=$DECODE_GPUS exec sglang serve "${COMMON[@]}" --disaggregation-mode decode \
      --disaggregation-bootstrap-port $BOOTSTRAP_PORT --port $DECODE_PORT --nccl-port 30400 "${RADIX[@]}" ;;
  lb)
